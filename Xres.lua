...
-- Variables untuk No Skillchecks Simple
...
local function isKillerTeam() 
    local tn = LocalPlayer.Team and LocalPlayer.Team.Name and LocalPlayer.Team.Name:lower() or "" 
...
-- Main Functions
local function blockSkillchecks()
    if not noSkillEnabled then return end
    
    -- Block skillcheck GUIs
    local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
    if playerGui then
        for _, gui in ipairs(playerGui:GetChildren()) do
            if gui.Name:lower():find("skill") or gui.Name:lower():find("qte") or gui.Name:lower():find("prompt") then
                pcall(function() gui:Destroy() end)
            end
        end
    end
    
    -- Block skillcheck remotes
    for _, obj in ipairs(ReplicatedStorage:GetDescendants()) do
        if obj:IsA("RemoteEvent") and (obj.Name:lower():find("skill") or obj.Name:lower():find("qte")) then
            spawn(function()
                pcall(function()
                    obj.OnClientEvent:Connect(function()
                        -- Block skillcheck events
                    end)
                end)
            end)
        end
...
ESPTab:CreateSection("World ESP Colors")
...
MiscTab:CreateToggle({
    ...
    Callback = function(state)
        noSkillEnabled = state
        if state then
            blockSkillchecks()
            Rayfield:Notify({
                Title = "No Skillchecks",
                Content = "Skillchecks blocked!",
                Duration = 3
            })
        else
            Rayfield:Notify({
...
-- Auto detect new GUIs untuk skillcheck blocking
LocalPlayer:WaitForChild("PlayerGui").ChildAdded:Connect(function(gui)
    if noSkillEnabled and (gui.Name:lower():find("skill") or gui.Name:lower():find("qte")) then
        wait(0.1) -- Tunggu sebentar biar GUI fully loaded
        pcall(function() gui:Destroy() end)
    end
end)
