...
function CreateColorPicker(initialColor, callback, parent)
    ...
    colorFrame.MouseButton1Click:Connect(function()
        -- Simple color picker using text input
...
function CreatePlayerTab(parent)
    ...
    list:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        tab.CanCanvasSize = UDim2.new(0, 0, 0, list.AbsoluteContentSize.Y)
    end)
end
...
function CreateVisualTab(parent)
    ...
    -- Name ESP Toggle
    CreateFeatureToggle("NAME ESP", "See player names", Xres.NameESP, tab, function(state)
        Xres.NameESP = state
        if Xres.ESP or Xres.NameESP then
            EnableESP()
        else
...
function CreatePerformanceTab(parent)
    ...
    infoLabel.Text = "FPS Boost disables shadows and reduces graphics quality to improve performance."
