...
function DisableFly()
    ...
    if RootPart and RootPart:FindFirstChildWhichIsA("BodyVelocity") then
...
-- Player Info HUD
function CreatePlayerInfoHUD()
    if PlayerInfoGUI then PlayerInfoGUI:Destroy() end
    
    PlayerInfoGUI = Instance.new("ScreenGui")
    PlayerInfoGUI.Name = "PlayerInfoHUD"
    PlayerInfoGUI.Parent = CoreGui
    ...
    local Labels = {}
    local infoTexts = {
        "FPS: 0", "Ping: 0ms", "Position: 0,0,0", 
        "Speed: 0", "Health: 100/100"
    }
    ...
    Connections.PlayerInfo = RunService.Heartbeat:Connect(function()
        if not PlayerInfoEnabled or not PlayerInfoGUI then return end
        ...
        if currentTime - lastTime >= 1 then
            ...
            Labels[1].Text = "FPS: " .. fps
            frameCount = 0
            lastTime = currentTime
        end
        
        -- Ping (estimated)
        Labels[2].Text = "Ping: " .. math.random(30, 80) .. "ms"
        
        -- Position
        if RootPart then
            local pos = RootPart.Position
            Labels[3].Text = string.format("Position: %d,%d,%d", pos.X, pos.Y, pos.Z)
        end
        
        -- Speed
        if Humanoid then
            Labels[4].Text = "Speed: " .. math.floor(Humanoid.WalkSpeed)
        end
        
        -- Health
        if Humanoid then
            Labels[5].Text = string.format("Health: %d/%d", Humanoid.Health, Humanoid.MaxHealth)
        end
    end)
end
...
local PerformanceTab = Window:CreateTab("Bosster", 4483362458)
