...
-- GANTI function registerFromDescendant dengan yang ini:
...
local autoSkillRemove = false
local skillConnection

MiscTab:CreateToggle({
    Name = "Auto Remove Skillchecks", 
    CurrentValue = false,
    Flag = "AutoNoSkill", 
    Callback = function(state)
        autoSkillRemove = state
        
        if state then
            -- Langsung di sini aja, gausah bikin function terpisah
            skillConnection = LocalPlayer.PlayerGui.ChildAdded:Connect(function(gui)
                if autoSkillRemove then
                    wait(0.3)
                    local nameLower = string.lower(gui.Name)
                    if nameLower:find("skill") or nameLower:find("qte") or nameLower:find("check") then
                        pcall(function() 
                            gui:Destroy() 
                        end)
                    end
            ...
            -- Remove skillchecks yang udah ada
            ...
            Rayfield:Notify({
                Title = "No Skillchecks", 
                Content = "Auto-remove deactivated",
                Duration = 3
            })
        end
    end
