...
local killerColors = {
    ...
    Abysswalker = Color3.fromRGB(255, 110, 60),
    ...
    for _,pl in ipairs(Players:GetPlayers()) do
        if pl ~= LP and getRole(pl)=="Killer" then
            if pl.Character then
                ...
                if head then
                    local tag = head:FindFirstChild("Xres_Tag")
                    if tag then
...
TabESP:CreateToggle({Name="Player ESP",CurrentValue=false,Flag="PlayerESP",Callback=function(s)
    playerESPEnabled=s
...
TabESP:CreateToggle({Name="Generators",CurrentValue=false,Flag="Gen",Callback=function(s) setWorldToggle("Generator", s) end})
TabESP:CreateToggle({Name="Hooks",CurrentValue=false,Flag="Hook",Callback=function(s) setWorldToggle("Hook", s) end})
TabESP:CreateToggle({Name="Gates",CurrentValue=false,Flag="Gate",Callback=function(s) setWorldToggle("Gate", s) end})
TabESP:CreateToggle({Name="Windows",CurrentValue=false,Flag="Window",Callback=function(s) setWorldToggle("Window", s) end})
TabESP:CreateToggle({Name="Pallets",CurrentValue=false,Flag="Pallet",Callback=function(s) setWorldToggle("Palletwrong", s) end})
TabESP:CreateToggle({Name="Pumpkins",CurrentValue=false,Flag="Pumpkin",Callback=function(s) setWorldToggle("Pumpkin", s) end})
TabESP:CreateSection("Color ESP")
...
TabESP:CreateSection("World Color")
TabESP:CreateColorPicker({Name="Generators",Color=worldColors.Generator,Flag="GenCol",Callback=function(c) worldColors.Generator=c end})
TabESP:CreateColorPicker({Name="Hooks",Color=worldColors.Hook,Flag="HookCol",Callback=function(c) worldColors.Hook=c end})
TabESP:CreateColorPicker({Name="Gates",Color=worldColors.Gate,Flag="GateCol",Callback=function(c) worldColors.Gate=c end})
TabESP:CreateColorPicker({Name="Windows",Color=worldColors.Window,Flag="WinCol",Callback=function(c) worldColors.Window=c end})
TabESP:CreateColorPicker({Name="Pallets",Color=worldColors.Palletwrong,Flag="PalCol",Callback=function(c) worldColors.Palletwrong=c end})
TabESP:CreateColorPicker({Name="Pumpkins",Color=worldColors.Pumpkin,Flag="PumpCol",Callback=function(c) worldColors.Pumpkin=c end})
        ...
        while anyWorldEnabled() do
            for cat,models in pairs(worldReg) do
                if worldEnabled[cat] then
                    local col, tagName, textName = worldColors[cat], "Xres_"..cat, "Xres_Text_"..cat
                    local n = 0
    ...
    if state then
        ...
        for _,entry in pairs(worldReg[cat]) do
            if entry and entry.part then
                clearChild(entry.part,"Xres_"..cat); clearChild(entry.part,"Xres_Text_"..cat)
            end
        end
    end
end
...
local function bindTimeLock()
    ...
    timeLockActive = true
    RunService:BindToRenderStep("Xres_TimeLock", 299, function()
...
TabPlayer:CreateButton({Name="Teleport to Killer",Callback=function() teleportToNearest("Killer") end})
TabPlayer:CreateButton({Name="Teleport to Team",Callback=function() teleportToNearest("Survivor") end})
...
TabWorld:CreateButton({Name="Instant-Escape",Callback=function()
...
Rayfield:Notify({Title="XresHub",Content="Loaded",Duration=6})