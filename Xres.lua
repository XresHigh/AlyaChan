...
function FindKiller()
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and getRole(player) == "Killer" then
            return player
        end
    end
    return nil
...
function FindSurvivor()
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and getRole(player) == "Survivor" then
            return player
        end
    end
    return nil
...
PlayerTab:CreateButton({
    ...
    Callback = function()
        ...
        if killer then
            local success = TeleportToPlayer(killer)
            if success then
                Rayfield:Notify({
                    Title = "Teleport",
                    Content = "Successfully teleported to Killer: " .. killer.Name,
                    Duration = 3
                })
            else
                Rayfield:Notify({
                    Title = "Teleport",
                    Content = "Failed to teleport to Killer!",
                    Duration = 3
                })
            end
        else
            Rayfield:Notify({
                Title = "Teleport",
                Content = "No Killer found in the game!",
                Duration = 3
            })
        end
    end,
...
PlayerTab:CreateButton({
    ...
    Callback = function()
        local survivor = FindSurvivor()
        if survivor then
            local success = TeleportToPlayer(survivor)
            if success then
                Rayfield:Notify({
                    Title = "Teleport",
                    Content = "Successfully teleported to Survivor: " .. survivor.Name,
                    Duration = 3
                })
            else
                Rayfield:Notify({
                    Title = "Teleport",
                    Content = "Failed to teleport to Survivor!",
                    Duration = 3
                })
            end
        else
            Rayfield:Notify({
                Title = "Teleport",
                Content = "No Survivor found in the game!",
                Duration = 3
            })
        end
    end,
...
-- Player Dropdown untuk teleport ke player tertentu
...
ESPTab:CreateSection("WORLD color")
...
Rayfield:Notify({
    Title = "XresHub",
    Content = "Successfully loaded with Player & World ESP!",
    Duration = 3
})